...
class TestLocalAssets(HttpCase):
    ...
    def test_repeated_url_fetched_once(self):
        ...
        html = f"<html><body>{images}</body></html>"
        with patch.object(
            AssetLocalizer, "_fetch", autospec=True, side_effect=AssetLocalizer._fetch
        ) as fetch:
            self.localizer().localize_html(html)
    ...
    def test_run_wkhtmltopdf_passes_local_html_and_allow_paths(self):
        ...
        self.assertNotIn(
            "--allow", self.report._build_wkhtmltopdf_args(paperformat, False)
        )
    ...
    def test_real_wkhtmltopdf_needs_no_http(self):
        ...
        html = (
            ...
            f'<link rel="stylesheet" href="{bundle.css()[0].url}"/>'
            f'<script src="{bundle.js()[0].url}"></script></head>'
            f'<body><img src="{STATIC_IMG}"/></body></html>'
        )
        ...
        with (
            ...
            pdf = self.report._run_wkhtmltopdf([html])
