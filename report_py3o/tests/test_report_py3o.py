...
class TestReportPy3o(TransactionCase):
    ...
    def _render_patched(self, result_text="test result", call_count=1):
        ...
        with mock.patch.object(
            ...
            patched_pdf.side_effect = (
                lambda record, data: py3o_report_obj._postprocess_report(
                    record, result
                )
                or result
            )
    ...
    def test_report_load_from_attachment(self):
        self.report.write(
            {"attachment_use": True, "attachment": "'my_saved_report'"}
        )
    ...
    def test_report_template_fallback_validity(self):
        ...
        self.report.py3o_template_fallback = flbk_filename
        self.assertFalse(
            self.py3o_report._get_template_from_path(flbk_filename)
        )
        with temporary_copy(flbk_filename) as tmp_filename:
            self.assertTrue(
                self.py3o_report._get_template_from_path(tmp_filename)
            )
        # check security
        self.assertFalse(
            self.py3o_report._get_template_from_path(
        ...
        self.assertFalse(
            self.py3o_report._get_template_from_path("/etc/test.odt")
        )
